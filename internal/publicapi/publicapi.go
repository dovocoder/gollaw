...
func AnalyzePublicAPI(ctx *analyzer.Context) (*apiReport, error) {
	...
	entries := collectExports(ctx)
	scanUses(ctx.Packages, entries)
	ownPkgUsage := buildOwnPackageUsageSet(ctx)

	report := &apiReport{TotalExports: len(entries)}
	classifyExports(ctx, entries, ownPkgUsage, report)
	sortExports(report)

	return report, nil
}

// collectExports gathers all exported identifiers from loaded packages.
func collectExports(ctx *analyzer.Context) map[string]*exportEntry {
...
// classifyExports splits entries into confirmed, accidental, and unused buckets.
func classifyExports(ctx *analyzer.Context, entries map[string]*exportEntry, ownPkgUsage map[string]bool, report *apiReport) {
...
// sortExports sorts each bucket by package then name.
func sortExports(report *apiReport) {
	less := func(s []exportInfo) func(i, j int) bool {
	...
	sort.Slice(report.ConfirmedPublic, less(report.ConfirmedPublic))
	sort.Slice(report.AccidentalExports, less(report.AccidentalExports))
	sort.Slice(report.UnusedExports, less(report.UnusedExports))
}
...
func FormatPublicAPIText(report *apiReport) string {
	...
	writeExportSection(&b, "Confirmed Public API (used by external importers)",
		"USED BY", report.ConfirmedPublic, false)
	writeExportSection(&b, "Accidental Exports (used only within own package — consider unexporting)",
		"FILE:LINE", report.AccidentalExports, true)
	writeExportSection(&b, "Unused Exports (not referenced anywhere)",
		"FILE:LINE", report.UnusedExports, true)

	return b.String()
}

// writeAPIHeader writes the public API report title and separator.
func writeAPIHeader(b *strings.Builder) {
	fmt.Fprintf(b, "Public API Report\n")
	fmt.Fprintf(b, "=================\n\n")
}

// writeExportSection writes a titled export table section to the builder.
// When showFileLine is true, the last column is "file:line"; otherwise "used by".
func writeExportSection(b *strings.Builder, title, lastColHeader string, exports []exportInfo, showFileLine bool) {
	if len(exports) == 0 {
		return
	}
	fmt.Fprintf(b, "%s\n", title)
	fmt.Fprintf(b, "%-30s  %-10s  %-40s  %s\n", "NAME", "KIND", "PACKAGE", lastColHeader)
	fmt.Fprintf(b, "%s\n", strings.Repeat("-", 100))
	for _, e := range exports {
		if showFileLine {
			fmt.Fprintf(b, "%-30s  %-10s  %-40s  %s:%d\n", e.Name, e.Kind, e.Package, e.File, e.Line)
		} else {
			fmt.Fprintf(b, "%-30s  %-10s  %-40s  %s\n", e.Name, e.Kind, e.Package, strings.Join(e.UsedBy, ", "))
		}
	}
	fmt.Fprintln(b)
}
