	...
	for _, cat := range cats {
		...
		for _, f := range fns {
			...
			fmt.Fprintf(&buf, "| %s | %s | %s:%d | %s | %s |\n",
				strings.ToUpper(string(f.Severity)), f.Analyzer, f.File, f.Line, f.Message, suggestion)
		}
		fmt.Fprintf(&buf, "\n</details>\n\n")
	}
	...
	for _, file := range files {
		...
		for _, f := range fns {
			...
			fmt.Fprintf(&buf, "| %s | %s | %d | %s | %s |\n",
				strings.ToUpper(string(f.Severity)), f.Analyzer, f.Line, f.Message, suggestion)
		}
		fmt.Fprintf(&buf, "\n")
	}
